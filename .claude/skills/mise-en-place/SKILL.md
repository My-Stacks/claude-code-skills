...
4. Write the run ledger `~/.claude/mise-en-place/<key>-last-run.md` (`mkdir -p` first). Minimum fields: `date`, `run_status`, `landed` (branch, old→new SHAs, PR numbers), `ticket_mutations` as a **count** so tomorrow's cap continues from it, `escalation_ticket` id if filed, `harvest` path, and `findings:` as `<class>/<surface>:<record-id>` ids. Prose alone silently resets the daily cap to zero.
