...
| Anything a user waits on | `openai/gpt-oss-120b:cerebras` | 1147 tok/s — the fastest model on the router, and near-cheapest at source |
...
- **`gpt-oss-120b` is simultaneously the fastest model on the router (1147 tok/s on
  Cerebras) and among the cheapest ($0.037 on DeepInfra).** Speed and price usually
  trade off; here they don't, because different providers serve the same weights.
  It earns its "default workhorse" slot on measurement, not vibes.
...
111 models on the router publish a price. Columns: **In/Out** = USD per 1M
...
| `prism-ml/Ternary-Bonsai-27B-AWQ-4bit` | 0.000 | 0.00 | 262k | VT·  | together | 80 |
...
| `inclusionAI/Ling-3.0-flash` | 0.060 | 0.18 | 262k | ·TS! | novita | 278 |
...
| `deepseek-ai/DeepSeek-V4-Flash-0731` | 0.080 | 0.18 | 1048k | ·TS! | deepinfra | 105 |
| `Qwen/Qwen3-32B` | 0.080 | 0.25 | 40k | ·TS! | nscale | 41 |
| `google/gemma-3-27b-it` | 0.080 | 0.16 | 131k | VTS  | deepinfra | 13 |
| `deepseek-ai/DeepSeek-V4-Flash` | 0.090 | 0.18 | 1048k | ·TS! | deepinfra | 84 |
...
| `tencent/Hy3` | 0.140 | 0.58 | 262k | ·TS  | deepinfra | 51 |
...
| `XiaomiMiMo/MiMo-V2.5` | 0.168 | 0.34 | 1048k | ·TS! | novita | 42 |
...
| `Qwen/Qwen3-VL-30B-A3B-Instruct` | 0.200 | 0.70 | 131k | VTS  | novita | 116 |
| `Qwen/Qwen3-Coder-30B-A3B-Instruct` | 0.228 | 0.91 | - | ·TS  | scaleway | 123 |
| `Qwen/Qwen3-235B-A22B-Thinking-2507` | 0.230 | 2.30 | 262k | ·TS! | deepinfra | 136 |
...
| `MiniMaxAI/MiniMax-M2.7` | 0.250 | 1.00 | 204k | ·T·  | deepinfra | 42 |
...
| `nvidia/NVIDIA-Nemotron-3-Ultra-550B-A55B-NVFP4` | 0.600 | 2.40 | 262k | ·TS  | fireworks-ai | 120 |
...
**36 of 111 accept images.** **2 are currently $0 in** (promos — never depend on one).
<!-- CATALOGUE:END -->
