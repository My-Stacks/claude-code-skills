...
| [vault-backup](./.claude/skills/vault-backup/) | Skill | 1.1 | Save research, project outputs, and knowledge artifacts from any Claude Code workspace into a shared Obsidian knowledge vault. |
...
| [housekeeping](./.claude/skills/housekeeping/) | Skill | 1.2 | Recovery command for when a project is in the weeds. Stops forward work, sweeps every configured surface (tickets, pages, CRM, transcripts, git, agent threads, artifacts), detects cross-surface contradictions via thirteen failure-mode detectors, harvests uncaptured session learnings, runs a Q&A prioritization round with the operator when a re-rank is needed, and returns a baseline readout with the next 3–5 moves — ordered, one owner and doneness test each, anchored to Linear tickets. All write-backs gated behind explicit approval. |
| [mise-en-place](./.claude/skills/mise-en-place/) | Skill | 1.2 | End-of-day shutdown ritual — the closing bookend to `preflight`. Lands the session's work up the ladder (uncommitted → committed → pushed → PR'd, **never merged**), sweeps git, GitHub, Linear, and the processes and scratch files the session started, reconciles surfaces that have drifted out of agreement, harvests traps and dead ends into the durable record, and always states what is still dirty. A three-posture charter split on the **network boundary**: commits and other local writes announce-then-act; anything crossing the wire (push, PR, ticket change, status update) stops and asks — push consent is remembered per repo, a PR is asked every night; merge, force-push, another person's files and another person's PR are never. Attribution comes from the session-start baseline `/preflight` leaves behind — absent, stale, or from a sibling worktree, the run degrades to report-only rather than guessing. Harvest never creates `journal/` in a repo that lacks one. `--dry-run` (forced on a repo's first closedown) and `--land-only` modes. Ends by calling `/linear handoff`. Requires `/preflight` to have run this session (a compacted session additionally needs a certified `/compact-clean` ledger to land anything); `git`; PR checks require authenticated `gh`; the Linear sweep requires the Linear MCP (skipped cleanly, and reported, if absent). |
| [compact-clean](./.claude/skills/compact-clean/) | Skill | 1.0 | Pre-compaction flush: run it right before `/compact`. Compaction destroys one thing, the session's knowledge that *it* made an edit, and without it `/mise-en-place` cannot tell your work from anything else that dirtied the tree. This certifies that authorship into an append-only local ledger under `~/.claude/compact-clean/`, with a content fingerprint per file, and saves decisions, traps and open threads to a notes file beside it. Records bind to the session mechanically through `CLAUDE_CODE_SESSION_ID`, so another session's record never lands, and a file edited by anything else after certification is rejected at closedown. Lands nothing and writes nothing into any repo. Ships a `PreCompact` hook that records uncertified evidence for auto-compaction, and a hermetic test suite. Requires `/preflight` to have run this session, `git` and `python3`. |
| [escape-hatch](./.claude/skills/escape-hatch/) | Skill | 1.0 | Escape hatch for leaving a terminal session mid-task without closing the work down. Commits and pushes everything that is yours as WIP (never a PR, never a merge), fast-forwards the bound Linear project to the current state through the `/linear` buffer (ticket states, comments, new tickets for unaddressed work), writes the handoff, and verifies that `/linear resume` can restart from the exact moment you left. One batched question at most. |
