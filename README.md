...
| [mise-en-place](./.claude/skills/mise-en-place/) | Skill | 1.0 | End-of-day shutdown ritual — the closing bookend to `preflight`. Lands the session's work up the ladder (uncommitted → committed → pushed → PR'd, **never merged**), sweeps git, GitHub, Linear, and the processes and scratch files the session started, reconciles surfaces that have drifted out of agreement, harvests traps and dead ends into the durable record, and always states what is still dirty. A three-posture charter governs every write: announce-then-act on ordinary landing, stop-and-ask on risky operations, never on merge, force-push, or another person's files. Attribution comes from the session-start baseline `/preflight` leaves behind — absent it, the run degrades to report-only rather than guessing. `--dry-run` and `--land-only` modes. Ends by calling `/linear handoff`. Requires `git`; PR checks require authenticated `gh`; the Linear sweep requires the Linear MCP (skipped cleanly, and reported, if absent). |
