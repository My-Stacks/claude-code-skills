...
| [compact-clean](./.claude/skills/compact-clean/) | Skill | 1.0 | Pre-compaction flush — run it right before `/compact`. Compaction destroys exactly one thing: the session's own knowledge that *it* made an edit, without which `/mise-en-place` cannot tell your work from anything else that dirtied the tree. This certifies that authorship and writes the session's live judgment (decisions, traps, dead ends, open threads) to an append-only local ledger at `~/.claude/compact-clean/`, keyed identically to the preflight baseline. Lands nothing and writes nothing into any repo, so it is safe to fire unattended. Ships a `PreCompact` command hook as a partial net for *auto*-compaction — a command hook has no model, so it records evidence marked `certified: false` and never licenses a commit. Requires `git` and `python3`. |
