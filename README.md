...
| [compact-clean](./.claude/skills/compact-clean/) | Skill | 1.0 | Pre-compaction flush: run it right before `/compact`. Compaction destroys one thing, the session's knowledge that *it* made an edit, and without it `/mise-en-place` cannot tell your work from anything else that dirtied the tree. This certifies that authorship into an append-only local ledger under `~/.claude/compact-clean/`, saves decisions, traps and open threads to a notes file beside it, and hands you the exact `/compact` line that keeps the record id in the summary; closedown trusts only a record whose id it can see, so another session's record never lands. Lands nothing and writes nothing into any repo. Ships a `PreCompact` hook that records uncertified evidence for auto-compaction. Requires `/preflight` to have run this session, `git` and `python3`. |
