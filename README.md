...
| [mise-en-place](./.claude/skills/mise-en-place/) | Skill | 1.0.1 | End-of-day shutdown ritual — the closing bookend to `preflight`. Lands the session's work up the ladder (uncommitted → committed → pushed → PR'd, **never merged**), sweeps git, GitHub, Linear, and the processes and scratch files the session started, reconciles surfaces that have drifted out of agreement, harvests traps and dead ends into the durable record, and always states what is still dirty. A three-posture charter split on the **network boundary**: commits and other local writes announce-then-act; anything crossing the wire (push, PR, ticket change, status update) stops and asks, with consent remembered per repo; merge, force-push, another person's files and another person's PR are never. Attribution comes from the session-start baseline `/preflight` leaves behind — absent, stale, or from a sibling worktree, the run degrades to report-only rather than guessing. Harvest never creates `journal/` in a repo that lacks one. `--dry-run` (run it first on any new repo) and `--land-only` modes. Ends by calling `/linear handoff`. Requires `/preflight` to have run this session; `git`; PR checks require authenticated `gh`; the Linear sweep requires the Linear MCP (skipped cleanly, and reported, if absent). |
