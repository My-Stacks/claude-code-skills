...
| [housekeeping](./.claude/skills/housekeeping/) | Skill | 1.0 | Recovery command for when a project is in the weeds. Stops forward work, sweeps every configured surface (tickets, pages, CRM, transcripts, git, agent threads, artifacts), detects cross-surface contradictions via thirteen failure-mode detectors, harvests uncaptured session learnings, runs a Q&A prioritization round with the operator when a re-rank is needed, and returns a baseline readout with the next 3–5 moves — ordered, one owner and doneness test each, anchored to Linear tickets. All write-backs gated behind explicit approval. |
