...
| [housekeeping](./.claude/skills/housekeeping/) | Skill | 1.1 | Recovery command for when a project is in the weeds. Stops forward work, sweeps every configured surface (tickets, pages, CRM, transcripts, git, agent threads, artifacts), detects cross-surface contradictions via thirteen failure-mode detectors, harvests uncaptured session learnings, runs a Q&A prioritization round with the operator when a re-rank is needed, and returns a baseline readout with the next 3–5 moves — ordered, one owner and doneness test each, anchored to Linear tickets. All write-backs gated behind explicit approval. |
| [hf-models](./.claude/skills/hf-models/) | Skill | 1.0 | Switch between open-weight models (Qwen, Kimi, DeepSeek, GLM, gpt-oss) through the Hugging Face Inference Providers router — one OpenAI-compatible endpoint, one token, model choice is a string. Live per-provider pricing lookup (the same model can differ ~9x by provider), single calls, parallel side-by-side comparison, and a task-to-model routing table with last-verified prices. Complements `ai-router` by adding open-weight models as a fourth, non-frontier-lab opinion. Requires `curl` and `python3`; an HF token with *Make calls to Inference Providers* permission for anything that calls out. |
